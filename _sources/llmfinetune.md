...
- Lors de la génération des réponses, le modèle peut être confronté au problème de l'hallucination, c'est-à-dire qu'il produit des réponses qui ne sont pas étayées par le contexte retrouvé. Cette phase peut également souffrir du manque de pertinence, de la toxicité ou de la partialité des résultats, ce qui nuit à la qualité et à la fiabilité des réponses.
...
En outre, il est à craindre que les modèles de génération ne s'appuient trop sur des informations enrichies, ce qui conduirait à des résultats qui reprennent simplement le contenu extrait sans y ajouter d'informations pertinentes ou synthétisées.
...
La figure ({numref}`ragmod`) illustre l'architectue des RAG modulaires. Cette dernière va au-delà des deux paradigmes RAG précédents, en offrant une adaptabilité et une polyvalence accrues. L'évolution vers une approche modulaire des RAG se généralise, prenant en charge à la fois le traitement séquentiel et l'entraînement de bout en bout. Malgré sa spécificité, le RAG modulaire s'appuie sur les principes fondamentaux du RAG avancé et du RAG naïf, illustrant les progrès sur cette thématique de recherche.
...
Le cadre modulaire RAG introduit des composants spécialisés supplémentaires pour améliorer les capacités de recherche et de traitement. Ainsi, par exemple : 
- le module *Recherche* s'adapte à des scénarios spécifiques, permettant des recherches directes dans diverses sources de données (moteurs de recherche, bases de données, graphes de connaissances,...), en utilisant le code généré par LLM et les langages d'interrogation. 
- le module *Fusion* s'attaque aux limites traditionnelles de la recherche en employant une stratégie multi-requête qui étend les requêtes de l'utilisateur à diverses perspectives, en utilisant des recherches vectorielles parallèles et un reclassement intelligent pour découvrir des connaissances à la fois explicites et transformatives. 
- le module *mémoire* exploite la mémoire du LLM pour guider la recherche, en créant une réserve de mémoire illimitée qui aligne le texte plus étroitement sur la distribution des données par le biais d'une auto-amélioration itérative.
- le module *Route* permet un routage de la requête  à travers diverses sources de données, sélectionnant le chemin optimal pour cette requête, qu'il s'agisse d'un résumé, de recherches dans des bases de données spécifiques ou de la fusion de différents flux d'informations. 
- le module *Prediction* vise à réduire la redondance et le bruit en générant un contexte directement par le biais du LLM, ce qui garantit la pertinence et la précision.  

Le RAG modulaire offre donc une remarquable capacité d'adaptation en permettant la substitution ou la reconfiguration de modules. Cela va au-delà des structures fixes des RAG naïfs et avancés, caractérisées par un simple mécanisme de « récupération » et de « lecture ». En outre, le RAG modulaire étend cette flexibilité en ajustant le flux d'interaction entre les modules existants, améliorant ainsi son applicabilité à différentes tâches.

Les ajustements de la disposition et de l'interaction des modules, tels que le cadre Demonstrate-Search-Predict (DSP)  et le flux itératif Retrieve-Read-Retrieve-Read d'ITER- RETGEN, montrent l'utilisation dynamique des sorties de modules pour renforcer la fonctionnalité d'un autre module, illustrant une compréhension sophistiquée de l'amélioration de la synergie entre les modules. Cette approche transcende le processus d'extraction fixe des RAG en évaluant la nécessité de l'extraction en fonction de différents scénarios. Un autre avantage d'une architecture flexible est que le système RAG peut plus facilement s'intégrer à d'autres technologies (telles que le fine tuning ou l'apprentissage par renforcement). 


### RAG vs. finetuning
Parmi les méthodes d'optimisation pour les LLM, le RAG est souvent comparé aux techniques de fine-tuning et à l'[ingénierie du prompt](./prompt.md). Chaque méthode présente des caractéristiques distinctes. L'ingénierie de prompt exploite les capacités inhérentes d'un modèle avec une nécessité minimale de connaissances externes et d'adaptation du modèle. La méthode RAG peut être comparée à un manuel de recherche d'informations adapté à un modèle, idéal pour des tâches précises de recherche d'informations. Le fine tuning est comparable à un étudiant qui intériorise ses connaissances au fil du temps, ce qui convient aux scénarios nécessitant la reproduction de structures, de styles ou de formats spécifiques.

Le RAG excelle dans les environnements dynamiques en offrant des mises à jour des connaissances en temps réel et une utilisation efficace des sources de connaissances externes avec une grande capacité d'interprétation. Cependant, il s'accompagne d'un temps de latence plus élevé et de considérations éthiques concernant l'extraction des données. Le fine-tuning est plus statique, nécessitant un réentraînement pour les mises à jour, mais permettant une personnalisation profonde du comportement et du style du modèle. Il exige des ressources informatiques importantes pour la préparation et l'entraînement des ensembles de données et, bien qu'il puisse réduire les hallucinations, il peut être confronté à des difficultés avec des données peu familières.
